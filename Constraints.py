...
class constraints:
    ...
    def rotational_constraint(self):
        p_i = [self.joints[self.body_index[self.i]][0], self.joints[self.body_index[self.i]][1],
               self.joints[self.body_index[self.i]][2]]
        p_before = [self.joints[self.body_index[self.i - 1]][0], self.joints[self.body_index[self.i - 1]][1],
                    self.joints[self.body_index[self.i - 1]][2]]
        p_next = [self.joints[self.body_index[self.i + 1]][0], self.joints[self.body_index[self.i + 1]][1],
                  self.joints[self.body_index[self.i + 1]][2]]

        v_i_next = [p_next[0] - p_i[0], p_next[1] - p_i[1], p_next[2] - p_i[2]]
        v_before_i = [p_i[0] - p_before[0], p_i[1] - p_before[1], p_i[2] - p_before[2]]

        dot_prod = np.inner(v_i_next, v_before_i)
        s = dot_prod
        ...
        if (self.constraint_type == "hinge"):
            ...
            unit_vec_i_o_array = [unit_vec_i_o[0], unit_vec_i_o[1], unit_vec_i_o[2]]
            # rotating p(i)-o C.C.W to find flexion constraint(left of pi_o)
            p_down = self.times(self.rotation_matrix(axis_normal_plane, self.theta[1]), unit_vec_i_o_array)
            p_down = [p_down[0], p_down[1], p_down[2]]
            ...
            if np.inner(np.cross(uv_i_before, unit_vec_i_o), (uv_normal_plane)) > 0:
                # means it is upper side of pi_o
                if self.theta[3] != 0:
                    # angle between vec(i_before) and vec(i_o)
                    angle = math.acos(np.inner(uv_i_before, unit_vec_i_o))
                    ...
                    p_nearest_point = [p_i[0] + unit_vec_i_o[0] * l_before_i,
    ...
    def q_matrix(self, x, y, a, b, x_t, y_t):
        Q = [[b ** 2 * x, a ** 2 * y], [(a ** 2 - b ** 2) * y + b ** 2 * y_t, (a ** 2 - b ** 2) * x - a ** 2 * x_t]]
        return Q

    def times(self, matrix, vector):
        return [
