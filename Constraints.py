...
# class constraints:
#     def __init__(self, joints, body_index, i, theta, length, constraint_type):
#         si = 0.0
#         pi = 22 / 7
#         theta = (pi / 180) * theta
#
...
#         v_i_next = CG3dVector(p_next_i[0] - p_i[0], p_next_i[1] - p_i[1], p_next_i[2] - p_i[2])
...
#             l_i_next_firstpose = utils.distance(p_i, p_next_i_first_pose)
#             v_i_next_firstPose = CG3dVector((-p_i[0] + p_next_i_first_pose[0]) / l_i_next_firstpose,
#                                             (-p_i[1] + p_next_i_first_pose[1]) / l_i_next_firstpose,
#                                             (-p_i[2] + p_next_i_first_pose[2]) / l_i_next_firstpose)
#             l_i_first_pose_before_firstpose = utils.distance(p_i, p_before_i)
#             v_before_first_pose_i_firstpose = CG3dVector(
#                 (-p_before_i[0] + p_i[0]) / l_i_first_pose_before_firstpose,
#                 (-p_before_i[1] + p_i[1]) / l_i_first_pose_before_firstpose,
#                 (-p_before_i[2] + p_i[2]) / l_i_first_pose_before_firstpose)
#
#             normal_vector_phi1 = v_i_next_firstPose ^ v_before_first_pose_i_firstpose
#
#             l_before_next_first_pose = utils.distance(p_before_i, p_next_i_first_pose)
#             v_before_next_first_pose = CG3dVector(
#                 (p_before_i[0] - p_next_i_first_pose[0]) / l_before_next_first_pose,
#                 (p_before_i[1] - p_next_i_first_pose[1]) / l_before_next_first_pose,
#                 (p_before_i[2] - p_next_i_first_pose[2]) / l_before_next_first_pose)
#
#             l1 = normal_vector_phi1 ^ v_before_next_first_pose
#
#             # for phi_2
#             l_before_next = utils.distance(p_before_i, p_next_i_first_pose)
#             v_next_before = CG3dVector((p_before_i[0] - p_next_i_first_pose[0]) / l_before_next,
#                                        (p_before_i[1] - p_next_i_first_pose[1]) / l_before_next,
#                                        (p_before_i[2] - p_next_i_first_pose[2]) / l_before_next)
#             v_l1 = CG3dVector(l1[0], l1[1], l1[2])
#
#             normal_vector_phi2 = v_next_before ^ v_l1
...
#             l_before_hat = utils.distance(p_before_i, p_hat)
#             unit_l2 = CG3dVector((p_hat[0] - p_before_i[0]) / l_before_hat,
#                                  (p_hat[1] - p_before_i[1]) / l_before_hat,
#                                  (p_hat[2] - p_before_i[2]) / l_before_hat)
#             p_prime_i = p_before_i + l_i_first_pose_before_firstpose * unit_l2
#
#             l_prime_next = utils.distance(p_prime_i, p_next_i)
#             v_prime_i_next = CG3dVector(p_next_i[0] - p_prime_i[0], p_next_i[1] - p_prime_i[1],
#                                         p_next_i[2] - p_prime_i[2])
#             # a unit vector of a line passing p(i-1) and P'(i)
#             unit_prime_next = CG3dVector(v_prime_i_next[0] / l_prime_next, v_prime_i_next[1] / l_prime_next,
#                                          v_prime_i_next[2] / l_prime_next)
#             p_next_prime = p_prime_i + l_i_next_firstpose * unit_prime_next
#
#             v_prime_i_next_prime = CG3dVector(p_next_prime[0] - p_prime_i[0], p_next_prime[1] - p_prime_i[1],
#                                               p_next_prime[2] - p_prime_i[2])
#
#             # finding o_hinge to know it is in limited zone
#             v_before_prime_i = CG3dVector(p_prime_i[0] - p_before_i[0], p_prime_i[1] - p_before_i[1],
#                                           p_prime_i[2] - p_before_i[2])
#             l_before_i_prime = utils.distance(p_prime_i, p_before_i)
#
#             l_prime_o = (v_before_prime_i * v_prime_i_next_prime) / l_before_i_prime
#
#             # a unit vector of a line passing p(i-1) and P'(i)
#             unit_vec_next_prime = CG3dVector((p_prime_i[0] - p_before_i[0]) / l_before_i_prime,
#                                              (p_prime_i[1] - p_before_i[1]) / l_before_i_prime,
#                                              (p_prime_i[2] - p_before_i[2]) / l_before_i_prime)
#
#             v_prime_o = l_prime_o * unit_vec_next_prime
#             o_hinge = CG3dPoint(v_prime_o[0] + p_prime_i[0], v_prime_o[1] + p_prime_i[1], v_prime_o[2] + p_prime_i[2])
#             s_hinge = utils.distance(p_prime_i, o_hinge)
#             q_2 = s_hinge * math.tan(theta[1])
#             q_4 = s_hinge * math.tan(theta[3])
#
#             # finding that the p_next is on right or left  of o:
#             cross_product = v_prime_i_next_prime ^ v_prime_o
#             angle_i_o_i_before = (math.sqrt(cross_product * cross_product)) / (l_prime_o * l_prime_next)
#
#             l_o_before_prime = utils.distance(o_hinge, p_next_prime)
#
#             # the distance between o and p_next_i:
#             if (angle_i_o_i_before <= 0):
#                 if (l_o_before_prime <= q_2):
#                     return p_next_prime
#                 # to return a nearest point on the circle
#                 else:
#                     circle_center = p_prime_i
#                     radius_circle = utils.distance(p_prime_i, p_next_prime)
#                     u = CG3dVector((p_next_prime[0] - p_prime_i[0]) / radius_circle,
#                                    (p_next_prime[1] - p_prime_i[1]) / radius_circle,
#                                    (p_next_prime[2] - p_prime_i[2]) / radius_circle)
#                     v = normal_vector_phi2 ^ u
#                     angle_difference = angle_i_o_i_before - theta[1]
#                     p_next_prime = circle_center + radius_circle * math.cos(
#                         angle_difference) * u + radius_circle * math.sin(angle_difference) * v
#                     return p_next_prime
#             elif (angle_i_o_i_before > 0):
...
#                                    (p_next_prime[2] - p_prime_i[2]) / radius_circle)
#                     v = normal_vector_phi2 ^ u
#                     angle_difference = pi / 2 + angle_i_o_i_before - theta[3]
#                     p_next_prime = circle_center + radius_circle * math.cos(
#                         angle_difference) * u + radius_circle * math.sin(angle_difference) * v
#                     return p_next_prime
#
...
#
#             # a unit vector of a line passing p(i-1) and P(i)
#             unit_vec_before_i = CG3dVector((p_i[0] - p_before_i[0]) / l_before_i,
#                                            (p_i[1] - p_before_i[1]) / l_before_i,
#                                            (p_i[2] - p_before_i[2]) / l_before_i)
#             # vector from p(i) to point o
#             v_i_o = l_i_o * unit_vec_before_i
#
...
#
#             l_o_next = utils.distance(o, p_next_i)
#             unit_v_o_next = CG3dVector((p_next_i[0] - o[0]) / l_o_next, (p_next_i[1] - o[1]) / l_o_next,
#                                        (p_next_i[2] - o[2]) / l_o_next)
#
#             y_t = l_o_next * math.sin(si)
#             x_t = l_o_next * math.cos(si)
#
...
#             diff_z_o_np = (l_o_nearest_point_in_ellipse_plane / l_o_next_in_ellipse_plane) * abs(o[2] - p_next_i[2])
#             if (unit_v_o_next[2] >= 0):
#                 z_nearest_point = o[2] + diff_z_o_np
#             else:
#                 z_nearest_point = o[2] - diff_z_o_np
#
#             p_nearest_point_in_global = CG3dPoint(o[0] + unit_v_o_next[0] * x_nearest_point,
#                                                   o[1] + unit_v_o_next[1] * y_nearest_point,
#                                                   o[2] + unit_v_o_next[2] * z_nearest_point)
#
#             return p_nearest_point_in_global
#
#     def func_point(self, x, a2, a1, a0):
#         return a2 * x * x + a1 * x + a0
#
#     def derivative_func_point(self, x, a2, a1):
#         return 2 * a2 * x + a1
#
...
class constraints:
    ...
    def __init__(self,joints, body_index, i, theta, length, constraint_type):
        ...
        #the rotation of the muscle
        self.si = 0.0

    def joint_location_calculator(self):
